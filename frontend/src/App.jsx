...
export default function App() {
  ...
  const handleSync = async (agentId, skillId) => {
    try {
      ...
      if (data.degraded) {
        showToast(`⚠️ 链接模式不可用，已降级为复制模式：「${skill?.name || skillId}」发往 ${agent?.name || agentId}`, "error");
      } else {
  ...
  const handleSyncAll = async (agentId) => {
    ...
    for (const skill of config.base.skills) {
      try {
        ...
        setConfig(data.config);
        setStatusMatrix(data.statusMatrix || {});
        if (data.degraded) degradedCount++;
      } catch (e) {
        showToast(`${skill.name}: ${e.message}`, "error");
      }
    }
    if (degradedCount > 0) {
      showToast(`⚠️ 已全量同步到 ${agent?.name}，其中 ${degradedCount} 个因链接不可用降级为复制`, "error");
    } else {
  ...
  const handleBatchSync = async () => {
    ...
    try {
      ...
      if (failCount > 0) {
        showToast(`已同步 ${skillIds.length} 个 skill 到 ${agentIds.length} 个 agent，${failCount} 个失败`, "error");
      } else if (data.degradedCount > 0) {
        showToast(`⚠️ 已同步 ${skillIds.length} 个 skill，其中 ${data.degradedCount} 个因链接不可用降级为复制`, "error");
      } else {
